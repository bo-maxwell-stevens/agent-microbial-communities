...
Cross-domain coupling was quantified using two complementary statistics: Mantel Spearman correlation for matrix-level concordance and Procrustes analysis for geometric alignment among ordination configurations. Coupling was evaluated at prevalence thresholds of 0.05 and 0.10 and summarized across threshold-specific estimates for each pair and representation branch. Mantel significance was assessed by permutation testing (999 permutations), and Procrustes uncertainty was quantified by bootstrap resampling (120 bootstraps). Conservative interval summaries were derived using 2.5th and 97.5th percentiles. Pairwise ranking was performed separately for Mantel and Procrustes metrics and then combined in an integrated coupling framework where coupling strength was defined as the mean of Mantel correlation and Procrustes similarity.

### 2.5 Environmental-driver analyses
Environmental structure in pairwise microbial correspondence was evaluated using distance-based redundancy analysis (dbRDA). Predictor sets were defined a priori to represent abiotic structure and plant-context covariates, with primary models including soil pH (pH_KCl), soil nitrogen percentage (N_pct), and precipitation (bio12; numeric scale 100), and geography-sensitivity models adding latitude and longitude to test spatially structured residual effects. Environmental analyses used a default prevalence threshold of 0.05, 999 permutations, and deterministic initialization with base seed 20260602. To preserve interpretability in predictor selection, model policy prohibited simultaneous inclusion of N_pct and C_pct. Predictor importance was quantified using leave-one-predictor reduction statistics (ΔR² and Δ adjusted R²), and environmental model adequacy was compared by adjusted R².

### 3.6 Plant-diversity hypothesis testing
Plant-diversity effects were tested against an abiotic baseline using seven pre-specified hypothesis models (A–G): A, abiotic baseline; B, abiotic+alpha diversity; C, abiotic+dark diversity; D, abiotic+species pool; E, abiotic+community completeness; F, abiotic+alpha+dark; and G, abiotic+pool+completeness. Plant-diversity analyses used a default threshold of 0.05, 999 permutations, base seed 20260603, CLR preprocessing with pseudocount 1×10⁻⁶, and 10 components for dimensional harmonization. Incremental explanatory value for each hypothesis was quantified as Δ adjusted R² relative to the abiotic baseline (model A), enabling direct comparison of alpha diversity, dark diversity, species-pool, and completeness contributions.

### 3.7 Integrated synthesis
Integrated synthesis combined three analytical layers for each pair and representation branch: (i) coupling strength from the Mantel–Procrustes framework, (ii) environmental explained variation from best-performing dbRDA formulations, and (iii) plant-diversity added variation from best non-baseline hypothesis models. For interpretive consistency, synthesis thresholds were prespecified as coupling strength ≥0.50, environmental adjusted R² ≥0.20, and plant-diversity Δ adjusted R² ≥0.01, with an additional low-plant criterion at Δ adjusted R² <0.005 to flag weak plant-associated increments. This framework provided a unified basis for cross-layer ranking and classification of ecological signal structure across domain pairings.

### 3.8 Statistical analyses
All statistical inference was based on completed outputs from fixed analytical workflows; no analyses were rerun for manuscript preparation. Significance testing for Mantel, dbRDA, and model-comparison procedures used permutation-based inference (999 permutations). Uncertainty for Procrustes-derived quantities was estimated with bootstrap resampling (120 replicates), and interval summaries were reported using percentile-based confidence limits where applicable (2.5th–97.5th). Across model layers, effect-size comparisons and rankings were based primarily on adjusted R² and Δ adjusted R² to support comparable interpretation under differing predictor sets.
