...
## Results
Figure 1 defines the shared matched cohort (n = 84), and Figures 2–5 organize the Results from coupling patterns to environmental structure, plant-diversity responses, and integrated synthesis.

### 3.1 Cross-domain coupling patterns
Cross-domain coupling was strongest for BAC↔ITS, especially in presence/absence space (coupling strength 0.574; Fig. 2; final_coupling_rankings.csv). EUK↔ITS remained close behind in both representations (0.538 and 0.535), showing that strong cross-domain concordance was not limited to one data representation. Across all 12 pair×representation combinations, coupling spanned 0.319–0.574, indicating broad but heterogeneous synchrony.

Mantel and Procrustes components captured complementary structure rather than a single ordering. BAC↔ITS presence/absence had the strongest Mantel correlation (0.584, p = 0.002) but ranked fifth for Procrustes similarity, whereas EUK↔ITS CLR had the highest Procrustes similarity (0.683) but ranked seventh by Mantel (phase5_bac_rank_summary.csv). This split shows that pairings with strong distance-matrix association were not always those with strongest geometric concordance.

The leading BAC↔ITS signal persisted across prevalence thresholds (Mantel 0.597 at 0.05 and 0.571 at 0.10), and AMF↔ITS CLR remained consistently positive (0.556 and 0.495). By contrast, BAC↔EUK presence/absence stayed weak (0.201 and 0.143) and was least significant at the more conservative threshold (p = 0.018 at 0.10; phase5_bac_mantel_inference.csv).

### 3.2 Environmental structure of coupled turnover
Environmental structure reinforced the coupling hierarchy by peaking in BAC↔ITS pairings (Fig. 3). In the final environmental synthesis, BAC↔ITS had the highest dbRDA adjusted R² values (0.278 in CLR; 0.265 in presence/absence), while the full integrated range across pairings was 0.188–0.278 (mean 0.223; final_environment_driver_summary.csv).

This environmental pattern was internally consistent across all integrated pairings. Geography-sensitivity was selected as the best environmental model in every case, pH_KCl was the top predictor in every case, and geography-sensitivity deltas were uniformly positive (0.00795–0.02189). pH contributions remained substantial throughout (0.09195–0.15099).

The full dbRDA output supported the same direction of effect: primary and geography-sensitivity models were significant across all pair×representation combinations (permutation p = 0.002), and geography-sensitivity adjusted R² exceeded primary adjusted R² in every pairing (phase5b_dbRDA_summary.csv; phase5b_pair_rankings.csv).

### 3.3 Plant-diversity contrasts across pairings
Plant-diversity effects were positive overall but shifted the cross-domain pattern toward AMF-linked pairings (Fig. 4). At the hypothesis level, the alpha-containing model B had the largest mean Δadjusted R² (0.01319), followed by F (0.01099) and D (0.00870), whereas the completeness-only model E was negative on average (-0.00373). All models were significant within each hypothesis summary (8/8; phase5c_hypothesis_summary.csv).

At the pair level, the largest plant-diversity increments occurred in AMF-linked relationships: AMF↔ITS presence/absence (0.01898), AMF↔EUK presence/absence (0.01716), and AMF↔ITS CLR (0.01650). BAC↔ITS had smaller increments (0.00893 in presence/absence; 0.00732 in CLR), so AMF↔ITS increments were approximately twofold larger than BAC↔ITS increments in both representations (final_plant_diversity_summary.csv).

Across all eight integrated entries, alpha, pool, and dark effects were positive, whereas completeness effects were negative in every case.

### 3.4 Integrated pattern across analytical layers
Integrating coupling, environmental structure, and plant-diversity increments (Fig. 5; final_pair_synthesis.csv) clarified a consistent cross-layer contrast. BAC↔ITS combined the strongest coupling and strongest environmental structure, but it showed the weakest plant-diversity increments. AMF-linked pairings showed the opposite profile: weaker coupling and lower-to-moderate environmental structure, but the strongest plant-diversity gains.

EUK↔ITS occupied an intermediate position, with high coupling, moderate environmental structure, and intermediate plant-diversity increments. Together, these layers converge on the same ecological pattern: BAC↔ITS dominates overall coupling and environmental organization, while AMF-linked relationships are most responsive to plant-diversity variation.
